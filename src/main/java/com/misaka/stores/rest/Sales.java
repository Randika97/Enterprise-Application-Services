...
public class Sales {
	...
	public Response getProducts(@Context SecurityContext sc, @Context UriInfo uriInfo) throws ParseException {
		...
		try {
			...
			StoredProcedureQuery procedureQuery = em.createNamedStoredProcedureQuery("Sales.GetSalesDetails");
			...
			if (firstRecord == 0 || paginationInfo) {
				...
				jpqlCountSb.append("Select count(distinct e.id) from Sales e ");
